...
│ <source>/hook.py                     │
│                                      │
│ Validates session belongs to source  │
│ Generates short ID:                  │
│   ide-{uuid[:8]}                     │
│   cli3-{uuid[:8]}                    │
│   cli2-{uuid[:8]}                    │
...
│   worker: "+name:desc" (sub-agent)   │
│                                      │
│ PostToolUse:                         │
│   event: "running"                   │
│   tool: "" (clear)                   │
│   file: "" (clear)                   │
│   worker: "-name" (sub-agent done)   │
│   tool_ok: true/false (exit code)    │
│   tool_result: "3/5: Fix bug"        │
│   description: <agent self-summary>  │
│   status: "in_progress" / etc        │
│   action_text: "? question"          │
│                                      │
│ Stop:                                │
│   event: "idle", priority: "low"     │
│   Clears: tool, file, action_text    │
│                                      │
│ UserPromptSubmit:                    │
│   event: "running"                   │
...
│                                      │
│ Deserializes EventUpdate from JSON   │
│ Persists to data/events/*.jsonl      │
│ Calls store.push_event(update)       │
│ If pin/mute → persist_session_meta   │
└──────────────────┬───────────────────┘
                   │
                   ▼
┌──────────────────────────────────────┐
│ state.rs — push_event()              │
│                                      │
│ Find session by ID or prefix match   │
...
│ Worker lifecycle:                    │
│   "+name" → workers[], sub_agents++  │
│   "-name" → remove, sub_agents--    │
│ Updates mtime, last_user_ts          │
...
│ → Re-render (panel updates list,     │
│   overlay re-syncs chars and modes)  │
└──────────────────────────────────────┘
```
...
│ scanner.rs — start()                 │
│                                      │
│ For each enabled source with cmd:    │
│ Spawns a background thread:          │
│   1. Runs scanner at startup         │
│   2. Loops: sleep(interval) → run    │
└──────────────────┬───────────────────┘
                   │ sh -c "<cmd>" (CWD=root)
                   ▼
┌──────────────────────────────────────┐
...
│   - state.vscdb SQLite (sessions)    │
│   - ~/.kiro/sessions/*/session.json  │
│                                      │
...
│                                      │
│ 1. Update meta on existing sessions  │
│    (name, workspace, group, tokens)  │
│    Never touches hook-owned fields!  │
│                                      │
│ 2. Create new sessions               │
│    - Hook fields start empty (None)  │
│    - Assign random char from pool    │
│    - Set mtime to now (if 0)         │
│                                      │
│ 3. GC dead sessions                  │
│    Not in report → removed           │
│                                      │
...
```
┌──────────────────────────────────────┐
│ attention.rs — compute() [every 5s]  │
│                                      │
│ For each active session:             │
│                                      │
│ IF event="tool" AND age>30s AND <1hr │
│   → attention=true, event="approval" │
│   → reason: "tool waiting {age}s"    │
│                                      │
│ ELIF event="running" AND age>120s    │
...
│                                      │
│ ELSE                                 │
│   → attention=false, clear all       │
│                                      │
│ Track attention_since (first set)    │
└──────────────────────────────────────┘
```
...
3. Start HTTP server + scanners + attention loop
4. Scanners immediately discover current sessions
   (this is the sole source of truth for sessions)
5. After scanners populate sessions:
   - Read data/sessions.json for metadata
   - Apply pinned/muted/title to matching sessions
   - Sessions not found by scanners are NOT restored
6. Clear stale attention (non-idle with attention)
7. Create overlay window (after 5s delay for Vite)
```

### Event Persistence (Debug Only)
...
- NOT used for startup recovery (scanners are sole source of truth)
